...
var penbuk = []domain.Penulis_Buku{
	{
...
var PubBuk = []domain.Penerbit_Buku{
	{
...
func (s *Seeds) Seed_Buku() {
	...
	for i, _ := range jenbuk {
		err := s.Db.Debug().Model(&domain.Jenis_Buku{}).Create(&jenbuk[i]).Error
		if err != nil {
	...
	for i, _ := range penbuk {
		err := s.Db.Debug().Model(&domain.Penulis_Buku{}).Create(&penbuk[i]).Error
		if err != nil {
			log.Fatalf("tidak bisa seed data: %v", err)
		}
	}
	...
	//penerbit buku
	for i, _ := range penbuk {
		err := s.Db.Debug().Model(&domain.Penerbit_Buku{}).Create(&PubBuk[i]).Error
		if err != nil {
			log.Fatalf("tidak bisa seed data: %v", err)
		}
	}
	...
	//buku
	for i, _ := range penbuk {
		err := s.Db.Debug().Model(&domain.Buku{}).Create(&dtbuku[i]).Error
		if err != nil {
			log.Fatalf("tidak bisa seed data: %v", err)
		}
	}
